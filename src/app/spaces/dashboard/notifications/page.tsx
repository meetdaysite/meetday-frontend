...
export default function SpaceNotificationsPage() {
	...
	const handleNotificationClick = (n: any) => {
		...
		const m = n.metadata || {}
		const tId = m.spaceThreadId || m.threadId || m.thread_id || m.interestId || m.interest_id || m.chatId || m.chat_id
		if (tId) {
			router.push(`/spaces/dashboard/chats?threadId=${tId}`)
		}
	}
			...
			<div className="flex-1 overflow-y-auto px-6 lg:px-8 py-8 max-w-4xl w-full mx-auto flex flex-col gap-6">
				...
				<div className="flex justify-between items-center pb-4 border-b border-black/10">
					<div>
						...
						<p className="text-sm font-semibold text-black/50 mt-1">Status updates, messages, and system alerts.</p>
					</div>
