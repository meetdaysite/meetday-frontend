...
export default function SpaceSponsorshipChatsPage() {
	...
	const loadThreads = useCallback(async () => {
		...
		try {
			...
			const byRecency = (a: SponsorshipChatThread, b: SponsorshipChatThread) => {
			...
			setAcceptedThreads([...acceptedData].sort(byRecency))
			setRequestedThreads([...requestedData].sort(byRecency))
		} catch {
	...
	useEffect(() => {
		if (selectedId) {
			setAcceptedThreads(prev => prev.map(t => (t.id === selectedId ? { ...t, unreadCount: 0 } : t)))
			setRequestedThreads(prev => prev.map(t => (t.id === selectedId ? { ...t, unreadCount: 0 } : t)))
	...
	const unreadAcceptedCount = acceptedThreads.reduce((sum, t) => sum + (selectedId === t.id ? 0 : Math.max(t.unreadCount || 0, getThreadUnreadCount(t.id))), 0)
	const unreadRequestedCount = requestedThreads.reduce((sum, t) => sum + (selectedId === t.id ? 0 : Math.max(t.unreadCount || 0, getThreadUnreadCount(t.id))), 0)
	...
	return (
		<div className="flex flex-col flex-1 min-h-0 bg-white">
			...
			<div className="flex-1 min-h-0 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 max-w-6xl w-full mx-auto flex flex-col gap-4">
				<div>
					<h1 className="text-2xl sm:text-3xl font-heading font-black text-black">Sponsorship Chats</h1>
					<p className="text-xs sm:text-sm font-semibold text-black/50 mt-1">
						Talk to brands interested in your sponsorship proposals.
					</p>
				</div>
				...
				<div className="h-[calc(100vh-240px)] border-[3px] border-black rounded-[24px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col sm:flex-row bg-white">
						...
						<div className="flex border-b-[3px] border-black shrink-0">
							{(["ACCEPTED", "REQUESTED"] as Segment[]).map(seg => {
								...
								return (
									...
									>
										<span>{seg === "REQUESTED" ? "Requests" : "Accepted"}</span>
										{count > 0 && (
											<span className={clsx(
						...
						<div className="flex-1 overflow-y-auto">
							{loadingThreads ? (
								...
								<p className="text-xs font-semibold text-black/40 text-center py-8 px-4">
									{segment === "REQUESTED" ? "No pending sponsorship requests." : "No accepted sponsorship chats yet."}
								</p>
							) : (
								threads.map(t => {
											...
											<div className="relative shrink-0">
												<div className="w-10 h-10 rounded-full border border-black/15 overflow-hidden bg-neutral-100 flex items-center justify-center">
													{t.counterpartAvatarUrl ? (
														...
														<span className="font-heading font-black text-sm text-black/60">{t.counterpartName.charAt(0).toUpperCase()}</span>
													)}
												</div>
												{unread > 0 && (
					...
					{/* Thread detail */}
					<div className={clsx("flex-1 min-w-0 min-h-0 flex flex-col", selectedId ? "flex" : "hidden sm:flex")}>
						{!selectedThread ? (
							<div className="flex-1 flex items-center justify-center text-sm font-semibold text-black/30">Select a chat to view</div>
						) : (
							<ChatThreadPanel key={selectedThread.id} thread={selectedThread} onAccept={handleAccept} onBack={() => setSelectedId(null)} />
						)}
					</div>
				</div>
			</div>
...
function ChatThreadPanel({
	...
	const ownName = profile?.businessName || "You"
	...
	const mentionSuggestions: MentionSuggestion[] = [
		{ id: "counterpart", name: thread.counterpartName, tag: thread.counterpartName.replace(/\s+/g, ""), role: "Brand", avatarUrl: thread.counterpartAvatarUrl },
		{ id: "meetday", name: "Meetday Support", tag: "Meetday", role: "Admin" },
	]
	...
	const { typingSenderType, notifyTyping, notifyStopTyping } = useChatTyping(thread.id, "SPACE")
	...
	const load = useCallback(async () => {
		...
		try {
			const [res, dealRes] = await Promise.all([
				getSponsorshipChatMessages(thread.id, "SPACE"),
			...
			if (prevMsgCountRef.current > 0 && res.messages.length > prevMsgCountRef.current) {
				...
				if (newest && newest.senderType !== "SPACE") {
					playMessageChime()
				}
			}
	...
	async function handleSend() {
		...
		setSending(true)
		try {
			const msg = await sendSponsorshipChatMessage(thread.id, { content: input.trim(), replyToId: replyingTo?.id, asRole: "SPACE" })
	...
	async function handleImagePick(e: React.ChangeEvent<HTMLInputElement>) {
		...
		try {
			...
			const msg = await sendSponsorshipChatMessage(thread.id, { mediaKey, replyToId: replyingTo?.id, asRole: "SPACE" })
			setMessages(prev => [...prev, msg])
			setReplyingTo(null)
		} catch {
	...
	function labelFor(senderType: SponsorshipChatMessage["senderType"]) {
		if (senderType === "SPACE") return `${ownName} • Space`
	...
	function replyLabel(senderType: SponsorshipChatMessage["senderType"]) {
		if (senderType === "SPACE") return ownName
	...
	return (
		<div className="flex-1 min-w-0 min-h-0 flex flex-col relative h-full bg-white">
				...
				<div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
					...
					<div className="w-8 h-8 rounded-full border border-black/15 overflow-hidden shrink-0 relative bg-neutral-100 flex items-center justify-center">
						{thread.counterpartAvatarUrl ? (
							<img src={thread.counterpartAvatarUrl} alt={thread.counterpartName} className="w-full h-full object-cover" />
						) : (
							<span className="font-heading font-black text-xs text-black/60">{thread.counterpartName.charAt(0).toUpperCase()}</span>
						)}
					</div>
			...
			<div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
				{loading ? (
					...
					<div className="m-auto text-center max-w-xs animate-in fade-in duration-200">
						...
						<p className="text-xs font-semibold text-black/50 mt-2">Accept the request to open the chat and reply.</p>
					</div>
				) : messages.length === 0 ? (
					...
					messages.map(m => {
						...
						const isMine = m.senderType === "SPACE"
									...
									{isDeleted ? (
										...
										<div
											className={clsx(
												...
												isSpaceMsg && "bg-[#FFC940] text-black",
												isBrand && "bg-[#EE2C2C] text-white",
												isAdmin && "bg-neutral-200 text-black",
												!isSpaceMsg && !isBrand && !isAdmin && "bg-neutral-200 text-black",
											)}
										>
											{m.replyTo && (
												<button
			...
			{dealModal === "report" && (
				<DealReportModal interestId={thread.id} role="SPACE" onClose={() => setDealModal(null)} />
			)}
