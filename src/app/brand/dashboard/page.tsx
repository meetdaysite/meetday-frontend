				...
				<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
					{isLoading && !data
						...
						: SUMMARY_CONFIG.map(({ status, countKey, label, subtitle, icon, iconColor, bg }) => (
							<Link
								key={status}
								href={`/brand/dashboard/events?status=${status}`}
				...
				<div className="flex flex-col gap-6 min-w-0">
					...
					<div className="bg-surface-card rounded-action border border-border-default shadow-card overflow-hidden">
						<div className="flex items-center justify-between px-5 py-4 border-b border-border-default">
							...
							<Link href="/brand/dashboard/events" className="text-label-sm text-text-brand hover:underline inline-flex items-center gap-1">
								View All Events
						...
						{isLoading && !data ? (
							...
							<div className="flex flex-col items-center justify-center py-16 gap-3">
								...
								<Link href="/brand/dashboard/create">
									<Button variant="primary" size="sm" radius="pill">Create your first campaign</Button>
								</Link>
							</div>
						) : (
										...
										{recentEvents.map((event, idx) => {
											...
											return (
												<tr key={event.id} className="hover:bg-surface-card-muted transition-colors">
													...
													<td className="px-4 py-3.5">
														<Link
															href={`/brand/dashboard/events/${event.id}`}
