...
export default function BrandLockedDealsPage() {
	...
	useEffect(() => {
		...
		getMySponsorshipChats("ACCEPTED", "BRAND")
			.then(async (threads) => {
				const dealsPromises = threads.map(async (thread) => {
					try {
						...
						if (deal && deal.status === "APPROVED") {
							...
							return {
								...deal,
								proposalName: thread.proposalName || "Untitled Project",
								communityName: thread.counterpartName,
	...
	return (
		<div className="flex flex-col min-h-full bg-white">
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 lg:px-8 py-5 border-b border-black/10 gap-2">
				<div>
					...
					<p className="text-xs font-semibold text-black/50 mt-0.5">
						All locked sponsorship agreements, deliverables reports, and payment statuses.
					</p>
				</div>
				<Link
			...
			<div className="px-6 lg:px-8 py-6 max-w-6xl mx-auto w-full flex-1 flex flex-col">
				{loading ? (
					...
					<div className="my-auto py-16 border-[3px] border-dashed border-black/30 rounded-[28px] bg-white flex flex-col items-center justify-center text-center p-8 gap-3 max-w-md mx-auto w-full">
						...
						<p className="text-xs font-semibold text-black/50 leading-relaxed">
							Once a sponsorship deal is approved and locked in your chats with a community, your active agreements and reports will appear here.
						</p>
						<Link
							href="/brand/dashboard/chats"
						...
						{lockedDeals.map((deal) => {
							...
							return (
								<div
									...
									className="group relative cursor-pointer bg-white border-[3px] border-black rounded-[20px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all p-5 flex flex-col justify-between"
								>
									<div>
										...
										<p className="text-xs text-black/60 font-semibold truncate mt-2">
											Project: <span className="text-black font-bold">{deal.proposalName}</span>
										</p>
									</div>
									<div className="mt-4 pt-3 border-t-2 border-black/5 flex flex-col gap-2">
										...
										<div className={clsx("grid gap-2 mt-1", deal.hasReport ? "grid-cols-2" : "grid-cols-1")}>
											<button
												...
												className="py-2 px-2 bg-[#FFC940] text-black border-2 border-black rounded-xl text-[10px] font-black tracking-wider shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all select-none text-center"
											>
											...
											{deal.hasReport && (
												<button
													...
													className="py-2 px-2 bg-white text-black border-2 border-black rounded-xl text-[10px] font-black tracking-wider shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all select-none text-center"
												>
													Report
												</button>
											)}
