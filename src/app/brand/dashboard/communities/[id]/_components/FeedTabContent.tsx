...
import { EditPostModal } from "@/app/communities/[slug]/_components/EditPostModal"
import {
...
function CreatePostCard({
	...
	const initials = displayName.split(" ").filter(Boolean).map(n => n[0]).slice(0, 2).join("").toUpperCase() || "H"
	...
	return (
		<div className="rounded-action bg-surface-card border border-border-default p-4 flex flex-col gap-3 shadow-md">
			{/* Input row */}
			<div className="flex items-start gap-3">
				{avatarUrl ? (
					<div className="relative size-10 rounded-full overflow-hidden shrink-0 border border-border-default bg-surface-hover mt-0.5">
						<Image src={avatarUrl} alt="You" fill sizes="40px" className="object-cover" />
					</div>
				) : (
					<div
				...
				<div className="relative flex-1">
					...
					<div className="flex items-end gap-2 px-4 py-2.5 rounded-action bg-surface-page border border-border-default transition-all">
						<textarea
							ref={textareaRef}
							rows={2}
							value={content}
			...
			<div className="flex items-center justify-between gap-2">
				<div className="flex items-center gap-2 flex-wrap">
					...
					>
						<div className="size-6 rounded-action bg-green-100 flex items-center justify-center shrink-0">
							<Icon as={GalleryWideSvg} size="xs" color="success" />
						</div>
						Photo / Video
					</button>
					<input
...
function PostCard({
	...
	const hostId = hostProfile?.id ?? null
	const isAuthor = hostId !== null && hostId === post.author.id
	const reactionCount = post.counts.reactions + (reacted !== post.reactedByMe ? (reacted ? 1 : -1) : 0)
	// The host always has moderation rights over their own community's feed.
	const isMod = true
	...
	return (
		<>
			<div
				ref={cardRef}
				className="rounded-action bg-surface-card border border-border-default p-4 flex flex-col gap-3 shadow-md"
			>
				{/* Clickable post body */}
					...
					<div className="border-t border-border-default pt-3 flex flex-col gap-3">
						...
						<div className="flex items-center gap-2">
							{hostProfile?.avatarUrl ? (
								...
								<div
									className={`size-7 rounded-full ${avatarColor(hostProfile?.displayName ?? "?").bg} border ${avatarColor(hostProfile?.displayName ?? "?").border} flex items-center justify-center shrink-0`}
								>
									<span
										className={`text-[9px] font-bold ${avatarColor(hostProfile?.displayName ?? "?").text}`}
									>
										{(hostProfile?.displayName ?? "?").split(" ").filter(Boolean).map(n => n[0]).slice(0, 2).join("").toUpperCase()}
									</span>
								</div>
							)}
						...
						{commentsLoading && comments.length === 0 ? (
							...
							<div className="flex flex-col gap-2.5">
								{comments.map(comment => {
									const isOwn = comment.author.id === hostId
...
export function FeedTabContent({ communityId }: { communityId: string }) {
	...
	return (
		<div className="flex flex-col gap-4">
			...
			{loading ? (
				...
				<div className="rounded-action bg-surface-card border border-border-default shadow-md p-10 flex flex-col items-center gap-2 text-center">
