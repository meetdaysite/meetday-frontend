...
function CommunityChatsContent() {
	...
	// Auto-align dealType and segment if a specific interestId was provided
	useEffect(() => {
		if (selectedId && (acceptedThreads.length > 0 || requestedThreads.length > 0)) {
			...
			if (match) {
				...
				if (requestedThreads.some(t => t.id === selectedId) && segment !== "REQUESTED") {
					setSegment("REQUESTED")
				} else if (acceptedThreads.some(t => t.id === selectedId) && segment !== "ACCEPTED") {
					setSegment("ACCEPTED")
				}
			}
		}
	}, [selectedId, acceptedThreads, requestedThreads, dealType, segment])
...
function ChatThreadPanel({
	...
	async function handleSend() {
		...
		setSending(true)
		try {
			const msg = await sendSponsorshipChatMessage(thread.id, { content: input.trim(), replyToId: replyingTo?.id, asRole: "HOST" })
	...
	async function handleImagePick(e: React.ChangeEvent<HTMLInputElement>) {
		...
		try {
			...
			const msg = await sendSponsorshipChatMessage(thread.id, { mediaKey, replyToId: replyingTo?.id, asRole: "HOST" })
			setMessages(prev => [...prev, msg])
			setReplyingTo(null)
		} catch {
