...
export function TicketCard({ ticket, tierIndex, quantity, onQuantityChange }: TicketCardProps) {
	...
	return (
		<div
			className={clsx(
				`rounded-panel border bg-surface-card p-4 flex items-center gap-4 transition-all duration-(--duration-180) border-l-4 border-l-${style.selectedBorder} hover:shadow-(--shadow-card-hover) shadow-md`,
				isSelected
