...
import {
	type SpaceHostDeal,
	type SpaceHostDealPayload,
	type SpaceHostDealStatus,
	type SpaceHostChatThread,
	type SpaceHostChatRole,
	type SpaceHostDealReport,
	isSpaceHostReportApproved,
	createSpaceHostDeal,
	updateSpaceHostDeal,
	approveSpaceHostDeal,
	requestSpaceHostDealChanges,
	getSpaceHostDeal,
	getSpaceHostDealReport,
	upsertSpaceHostDealReport,
} from "@/lib/api"
import { uploadSpaceHostDealReportImage } from "@/lib/uploadMedia"
import { ImageLightbox } from "@/components/ui/ImageLightbox"

const STATUS_LABEL: Record<SpaceHostDealStatus, string> = {
	PENDING_APPROVAL: "Pending Approval",
	CHANGES_REQUESTED: "Changes Requested",
	APPROVED: "Locked",
}

const STATUS_COLOR: Record<SpaceHostDealStatus, string> = {
...
export function SpaceHostDealBanner({
	deal,
	role,
	onLock,
	onEdit,
	...
	onReport?: () => void
	hasReport?: boolean
	report?: SpaceHostDealReport | null
}) {
	...
	const isClosed = isSpaceHostReportApproved(report)
...
// ─── Space Host Deal Form Modal (Lock/Edit Deal) ──────────────────────────────

const EMPTY_FORM: SpaceHostDealPayload = {
	projectName: "",
	goals: "",
	venue: "",
	time: "",
...
export function SpaceHostDealFormModal({
	interestId,
	deal,
	role,
	onClose,
	...
	interestId: string
	thread?: SpaceHostChatThread | null
	deal: SpaceHostDeal | null
	role?: SpaceHostChatRole
	onClose: () => void
	onSaved: (deal: SpaceHostDeal) => void
}) {
	const [form, setForm] = useState<SpaceHostDealPayload>(
		deal
			? {
	...
	function getMissingFieldLabels(): string[] {
		...
		if (typeof form.goals === "string" ? !form.goals.trim() : !(form.goals && form.goals.length > 0)) missing.push("Goals")
	...
	async function handleSubmit() {
		...
		try {
			...
			const payload: SpaceHostDealPayload = {
				...form,
				sponsorshipAmount,
			...
			const saved = deal
				? await updateSpaceHostDeal(interestId, payload, role)
				: await createSpaceHostDeal(interestId, payload, role)

			toast.success(deal ? "Deal updated." : "Deal locked — waiting for the community's approval.")
			onSaved(saved)
			onClose()
		} catch (err: any) {
				...
				<div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3.5">
					<Field label="Project Name" required>
						<input
							...
							placeholder="e.g. Summer Community Pop-up"
						/>
					</Field>

					<Field label="Goals" required hint="e.g. *Brand Activation, *Event, *Branding">
						<input
...
// ─── Space Host Deal Details Modal (View/Approve/Request Changes) ─────────────

export function SpaceHostDealDetailsModal({
	interestId,
	deal,
	role,
	onClose,
	...
	role: SpaceHostChatRole
	onClose: () => void
	onUpdated: (deal: SpaceHostDeal) => void
}) {
	const [requestingChanges, setRequestingChanges] = useState(false)
	const [note, setNote] = useState("")
	const [busy, setBusy] = useState(false)

	// The community (HOST) can approve / request changes.
	const canApproveOrRequestChanges = role !== "SPACE"

	async function handleApprove() {
		setBusy(true)
		try {
			const updated = await approveSpaceHostDeal(interestId, role)

			toast.success("🎉 Deal approved and locked!")

			const canvas = document.getElementById("space-host-chat-confetti-canvas") as HTMLCanvasElement | null
			if (canvas) {
	...
	async function handleRequestChanges() {
		...
		setBusy(true)
		try {
			const updated = await requestSpaceHostDealChanges(interestId, { note: note.trim() }, role)
		...
		>
			<div className="bg-white rounded-[24px] border-[3px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] w-full max-w-lg flex flex-col max-h-[90vh]">
				...
				<div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3.5 text-sm">
					...
					{deal.goals && <Row label="Goals" value={typeof deal.goals === "string" ? deal.goals : deal.goals.join(", ")} />}
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
...
// ─── Space Host Deal Report Modal (Deliverables Report) ───────────────────────

export function SpaceHostDealReportModal({
	interestId,
	role,
	onClose,
	onReportUpdated,
	...
	role: SpaceHostChatRole
	onClose: () => void
	onReportUpdated?: (report: SpaceHostDealReport) => void
}) {
	...
	useEffect(() => {
		Promise.all([
			getSpaceHostDealReport(interestId, role).catch(() => null),
			getSpaceHostDeal(interestId).catch(() => null),
		])
			.then(([r, d]) => {
				setDeal(d)
				setReport(r)
	...
	async function handleAddImage(file: File) {
		...
		setUploading(true)
		try {
			const key = await uploadSpaceHostDealReportImage(file, interestId)
	...
	async function handleSubmit() {
		...
		try {
			...
			const saved = await upsertSpaceHostDealReport(
				interestId,
				{
					projectName: projectName.trim(),
					eventDate: date.trim(),
