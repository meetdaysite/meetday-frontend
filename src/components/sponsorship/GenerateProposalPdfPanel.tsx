...
import { generateProposalPdf, type GenerateProposalPdfPayload } from "@/lib/api"
...
// Standalone "quick pitch" PDF generator — separate from the full Create Proposal flow;
// no saved record, just a form-in/PDF-out download. Field set is a first pass covering the
// standard fields (sponsor, deliverables, timeline, pricing, terms, contact) — expected to be
// refined once exact requirements are confirmed.
export function GenerateProposalPdfPanel({ onClose }: GenerateProposalPdfPanelProps) {
	...
	async function handleGenerate(e: React.FormEvent) {
		...
		setGenerating(true)
		try {
			const payload: GenerateProposalPdfPayload = {
			...
			a.href = url
			a.download = "sponsorship-proposal.pdf"
	...
	return (
		<div className="animate-in fade-in duration-150 flex flex-col gap-6">
			<div className="flex justify-between items-center shrink-0">
				<div className="flex flex-col gap-1">
					...
					<p className="text-sm font-semibold text-black/50 mt-1">
						Fill in the pitch details and download a styled PDF to share directly
					</p>
				</div>
				<button
					type="button"
			...
			<form onSubmit={handleGenerate} className="border-[3px] border-dashed border-black/30 rounded-[28px] p-6 bg-white flex flex-col gap-6 w-full">
