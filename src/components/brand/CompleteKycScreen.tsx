...
import { getBrandProfile, updateBrandProfile, verifyPan, verifyBankAccount, type BrandProfile } from "@/lib/api"
...
export function CompleteKycScreen({ profile, onSignOut }: { profile: BrandProfile; onSignOut: () => void }) {
	...
	async function onSubmitPan(values: PanFormValues) {
		...
		setPanLoading(true)
		try {
			await updateBrandProfile({ pan: values.pan, legalName: values.legalName })
